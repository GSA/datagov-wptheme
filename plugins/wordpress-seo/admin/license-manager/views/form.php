	...
	<table class="form-table yoast-license-form">
		...
		<tr valign="top">
			...
			<td>
				<?php if ( $this->license_is_valid() ) { ?>
					<span
						style="color: white; background: limegreen; padding:3px 6px;">ACTIVE</span> &nbsp; - &nbsp; you are receiving updates.
				<?php } else { ?>
					<span
						style="color:white; background: red; padding: 3px 6px;">INACTIVE</span> &nbsp; - &nbsp; you are
		...
		<tr valign="top">
			...
			<td class="yoast-license-toggler">
				...
				<?php if ( $this->license_is_valid() ) { ?>
					<button name="<?php echo esc_attr( $action_name ); ?>" type="submit"
					        class="button-secondary yoast-license-deactivate"
					        value="deactivate"><?php echo esc_html_e( 'Deactivate License', $product->get_text_domain() ); ?></button> &nbsp;
				...
				<?php
					...
					if ( $this->get_license_key() !== '' ) {
						?>
						<button name="<?php echo esc_attr( $action_name ); ?>" type="submit"
						        class="button-secondary yoast-license-activate"
						        value="activate" /><?php echo esc_html_e( 'Activate License', $product->get_text_domain() ); ?></button> &nbsp;
					<?php
					} else {
		...
		<tr valign="top">
			...
			<td>
				<input name="<?php echo esc_attr( $key_name ); ?>" type="text"
				       class="regular-text yoast-license-key-field <?php if ( $obfuscate ) { ?>yoast-license-obfuscate<?php } ?>"
				       value="<?php echo esc_attr( $visible_license_key ); ?>"
				       placeholder="<?php echo esc_attr( sprintf( __( 'Paste your %s license key here..', $product->get_text_domain() ), $product->get_item_name() ) ); ?>" <?php if ( $readonly ) {
