...
if ( ! class_exists( 'CustomContactFormsAdmin' ) ) {
	class CustomContactFormsAdmin extends CustomContactForms {
		...
		function insertAdminScripts() {
			...
			$js_ajax       = array(
				'plugin_dir' => plugins_url() . '/custom-contact-forms',
		...
		function printAdminPage() {
			...
			<div id="create-fields" class="postbox">
				...
				<div class="inside">
					<form id="ccf-create-field" method="post"
...
//			$field_types = str_replace('<option>'.$fields[$i]->field_type.'</option>',  '<option selected="selected">'.$fields[$i]->field_type.'</option>', $field_types);
						...
						<tr class="row-field-<?php echo esc_attr( $fields[ $i ]->id ); ?> <?php if ( $z % 2 == 0 )
							...
							<td><?php if ( $fields[ $i ]->field_slug == 'resetButton' ) {
									...
									<input type="text"
									       name="objects[<?php echo esc_attr( $fields[ $i ]->id ); ?>][values][field_label]"
									       maxlength="100"
									       value="<?php echo esc_attr( $fields[ $i ]->field_label ); ?>"/>
								<?php } ?></td>
			...
			foreach ( $styles as $style ) {
				...
				<td><label><input type="checkbox" class="object-check" value="1"
					...
					<select name="objects[<?php echo esc_attr( $style->id ); ?>][values][submit_background_repeat]">
						...
						<option <?php selected( 'repeat-x', $style->submit_background_repeat ); ?> value="repeat-x">
							Repeat
							X
						</option>
						<option <?php selected( 'repeat-y', $style->submit_background_repeat ); ?> value="repeat-y">
							Repeat
							Y
						</option>
				...
				<td><label>
					...
					<input type="text" maxlength="20"
					       value="<?php echo esc_attr( $style->success_popover_fontsize ); ?>"
				...
				<form class="ccf-edit-ajax" method="post" action="<?php echo esc_url( $_SERVER['REQUEST_URI'] ); ?>">
					<table class="widefat post" id="form-submissions-table" cellspacing="0">
						...
						foreach ( $user_data_array as $data_object ) {
							$data = new CustomContactFormsUserData( array(
								'form_id'      => $data_object->data_formid,
								'data_time'    => $data_object->data_time,
								'form_page'    => $data_object->data_formpage,
		...
		function printSettingsPage() {
			...
			if ( isset( $_POST['general_settings'] ) ) {
				$_POST['settings']                       = array_map( array(
					'ccf_utils',
					'encodeOption'
				), $_POST['settings'] );
			...
			<ul class="gright">
				...
				<li>
					...
					<input name="settings[default_form_bad_permissions]"
					       value="<?php echo esc_attr( $admin_options['default_form_bad_permissions'] ); ?>"
					       type="text"/>
				</li>
				<li class="descrip">
