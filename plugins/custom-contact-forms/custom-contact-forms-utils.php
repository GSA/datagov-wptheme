...
if ( ! class_exists( 'ccf_utils' ) ) {
	class ccf_utils {
		...
		static function decodeOption( $option, $strip_slashes = 1, $decode_html_chars = 1 ) {
			if ( $strip_slashes == 1 ) {
				$option = stripslashes( $option );
			}
			if ( $decode_html_chars == 1 ) {
		...
		function defineConstants() {
			...
			$GLOBALS['ccf_tables_array'] = array(
				CCF_FORMS_TABLE,
				CCF_FIELDS_TABLE,
				CCF_STYLES_TABLE,
				CCF_USER_DATA_TABLE,
				CCF_FIELD_OPTIONS_TABLE
			);
			$GLOBALS['ccf_fixed_fields'] = array(
				'customcontactforms_submit' => '',
				'fid'                       => '',
