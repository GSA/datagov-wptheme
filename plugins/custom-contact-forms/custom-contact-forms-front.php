...
if ( ! class_exists( 'CustomContactFormsFront' ) ) {
	class CustomContactFormsFront extends CustomContactForms {
		...
		function insertDatePickerScripts() {
			wp_enqueue_script( 'jquery-ui-datepicker', plugins_url() . '/custom-contact-forms/js/jquery.ui.datepicker.js', array(
				'jquery-ui-core',
				'jquery-ui-widget'
			) );
		...
		function insertFormSuccessCode() {
			...
			if ( $form->form_style != 0 ) {
				...
				<style type="text/css">
					#ccf-form-success {
						z-index: 10000;
						border-color: # <?php echo esc_attr(parent::formatStyle($style->success_popover_bordercolor)); ?>;
						height: <?php echo esc_attr($style->success_popover_height); ?>;
					}

					#ccf-form-success div {
						background-color: # <?php echo esc_attr(parent::formatStyle($style->success_popover_bordercolor)); ?>;
					}

					#ccf-form-success div h5 {
						color: # <?php echo esc_attr(parent::formatStyle($style->success_popover_title_fontcolor)); ?>;
						font-size: <?php echo esc_attr($style->success_popover_title_fontsize); ?>;
					}

					#ccf-form-success div a {
						color: # <?php echo esc_attr(parent::formatStyle($style->success_popover_title_fontcolor)); ?>;
					}

					#ccf-form-success p {
						font-size: <?php echo esc_attr($style->success_popover_fontsize); ?>;
						color: # <?php echo esc_attr(parent::formatStyle($style->success_popover_fontcolor)); ?>;
					}
				</style>
			<?php
			}
		...
		function getFormCode( $form, $is_widget_form = false ) {
			...
			foreach ( $fields as $field_id ) {
				...
				// Do not show these fields to authorized users (e.g. Name, Email, username etc)
				if ( false !== strpos( $field->field_class, 'hide-for-auth' ) && is_user_logged_in() ) {
					continue;
				}
		...
		function processFileUpload( $field ) {
			$errors = array();
			if ( empty( $_FILES[ $field->field_slug ] ) ) {
			...
			if ( ! empty( $allowed_exts ) ) {
				if ( ! in_array( $ext, $allowed_exts ) ) {
					$errors[] = '.' . $ext . __( ' is an invalid file extension.', 'custom-contact-forms' );
				}
			}
			if ( ! empty( $errors ) ) {
				return $errors;
		...
		function processForms() {
			...
			if ( isset( $_POST['ccf_customhtml'] ) ) {
				...
				if ( empty( $errors ) ) {
					...
					$data_object = new CustomContactFormsUserData( array(
				...
				$admin_options      = parent::getAdminOptions();
				$fields             = parent::getAttachedFieldsArray( $_POST['fid'] ); // unserialize($form->form_fields);
				$post_time          = time();
...
//                    load field as obj
					...
					if ( false !== strpos( $field->field_class, 'hide-for-auth' ) ) {
						if ( is_user_logged_in() ) {
							 ...
							 */
							! isset( $current_user ) ? ( $current_user = wp_get_current_user() ) : true;
					...
					} elseif ( $field->field_slug == 'your_email' && $field->field_required == 1 && ! empty( $_POST['your_email'] ) ) {
						if ( ! is_user_logged_in() && ! $this->validEmail( $_POST['your_email'] ) ) {
							if ( empty( $field->field_error ) ) {
								$this->setFormError( 'your_email', __( 'The email address you provided is not valid.', 'custom-contact-forms' ) );
							} else {
								$this->setFormError( 'your_email', $field->field_error );
							}
						}
...
//                        REQUIRED && NOT A FILE && POST FIELD FOUND - PROCESS DROPDOWN & RADIO & CHECKBOX
...
//                            REQUIRED NON-FILE FIELD NOT FOUND
							if ( empty( $field->field_error ) ) {
								$this->setFormError( $field->field_slug, $lang['field_blank'] . '"' . $field_error_label . '"' );
							} else {
								$this->setFormError( $field->field_slug, $field->field_error );
